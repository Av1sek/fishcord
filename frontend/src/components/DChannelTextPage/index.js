...
const DChannelTextPage = () => {
    ...
    const { dchannelId } = useParams();
    const sessionUser = useSelector(state => state.session.user)
    const [messagesLoaded, setMessagesLoaded] = useState(false);
    const dchannels = useSelector(state => state.dchannels)
    const stateDmessages = useSelector(state => state.dmessages)
    ...
    useEffect(() => {
        let cable = createConsumer("ws://fishcord.herokuapp.com/cable");
        ...
        return function cleanup() {
            console.log("unsubbing from ", dchannelId)
    ...
    const handleDelete = (dmessageId) => {
        dispatch(deleteDmessage(dmessageId));
    }
    ...
    const handleUpdateMessage = (e) => {
        e.preventDefault();
        dispatch(updateDmessage({id: editId, content: editText, author_id: sessionUser.id, chatroom_id: dchannelId, author_name: sessionUser.username})).then(() => {
            cancelEditDiv()
        })
    }
 
    const messagesList = Object.values(stateDmessages).map((message) => (
    ...
    const sendMessage = (e) => {
        e.preventDefault();
        dispatch(createDmessage({content: content, author_id: sessionUser.id, chatroom_id: dchannelId, author_name: sessionUser.username})).then(() => {
...
export default DChannelTextPage;